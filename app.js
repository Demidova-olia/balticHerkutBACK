...
const LOCAL_NET_5173 = [/^http:\/\/192\.168\.\d+\.\d+:5173$/i, /^http:\/\/10\.\d+\.\d+\.\d+:5173$/i];
...
if (process.env.CLOUDINARY_URL) {
  ...
  if (!process.env.CLOUDINARY_API_KEY || !process.env.CLOUDINARY_API_SECRET) {
    console.warn("[cloudinary] WARNING: API credentials are missing. Uploads may fail.");
  } else {
...
const server = app.listen(PORT, () =>
  console.log(`Server is running on port ${PORT} (NODE_ENV=${process.env.NODE_ENV || "dev"})`)
);
